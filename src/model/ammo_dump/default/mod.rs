...
impl Damageable for DefaultAmmoDump {
  fn add_damage(
    ...
    if let State::Nominal(state_transition) = &self.state {
      self.state = state_transition.to_exploding();
      self.updated = true;
    }
  }
}
...
impl Model for DefaultAmmoDump {
  fn update(
    ...
    match &self.state {
      State::Cooling(state_transition) => {
        self.cooling_time_elapsed_seconds += time_delta;
        if self.cooling_time_elapsed_seconds >= AMMO_DUMP_COOLING_TIME_SECONDS {
          self.state = state_transition.to_nominal();
        }
      },
      State::Exploding(state_transition) => {
        self.state = state_transition.to_cooling();
      ...
      State::Nominal(_state_transition) => {
